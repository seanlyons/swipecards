...
func twitter_buttons():
	var t = %Util.get_tweets(randi_range(0, 13000))
	for ts in t:
		%Hallmark.db_row_to_cardmaker(ts)

func cprint_base(text:String, complex:bool):
	if complex:
		#JSON.print(text, "  ")
		pass
	emit_signal("output_received", text)	

func ccprint(text:String):
	cprint_base(text, true)

func cprint(text : String) -> void:
	cprint_base(text, false)
